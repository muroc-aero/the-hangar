...
| `hangar-avy` | `hangar.avy` | NASA Aviary coupled aircraft-sizing + mission optimization server (FLOPS/GASP methods on OpenMDAO/dymos). Installed into the shared workspace venv by `scripts/dev-setup.sh` -- see `packages/avy/CLAUDE.md`. | `avy-cli`, `avy-server` |
