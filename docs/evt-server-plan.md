# Plan: hangar-evt — evtolpy MCP server + CLI

Add `packages/evt/` wrapping [starbelt/evtolpy](https://github.com/starbelt/evtolpy)
...
- **API**: one entry point — `evt.aircraft.Aircraft(path_to_cfg.json)`.
...
- **Packaging**: none — no `setup.py`/`pyproject.toml`, not on PyPI. Only
  hard dependency is matplotlib. `evt/` has an `__init__.py`, so it is a
...
3. **Packaging shim**: evtolpy has no project metadata, so the
   ...
   ref, `packages = ["evt"]`, dependency `matplotlib`). Apply/reverse-apply
...
4. Smoke test: `uv sync` then
   `python -c "from evt.aircraft import Aircraft"`.
5. **Dockerfile** for `packages/evt/` must clone+patch the pinned upstream
   the same way the ocp/pyc Dockerfiles handle theirs (ARG default from
   `upstream-pins.env`).

## Phase 1 — package skeleton

`packages/evt/` mirroring ocp/pyc layout (namespace rule: `__init__.py`
only at `src/hangar/evt/`, never `src/hangar/`):
...
├── skills/evt-cli-guide/  # SKILL.md, commands.md, modes.md,
...
[project.scripts]
evt-server = "hangar.evt.server:main"
evt-cli    = "hangar.evt.cli:main"

[project.entry-points."hangar.study_runners"]
evt = "hangar.evt.study_runner"
...
registered through `capture_tool`, resources `evt://reference`,
`evt://workflows`, `evt://artifacts/{run_id}`, prompts for guided
workflows, `main()` via `run_server_main(mcp, tool="evt",
env_prefix="EVT", default_port=8004)`.

`cli.py`: `build_evt_registry()` + `main()` via
...
`packages/evt/examples/mission_segments/` follows the ocp/pyc lane
...
├── lane_b/              # hangar-evt MCP tools via JSON scripts
...
└── tests/
    ...
    └── test_cli_script.py  # evt-cli script mode runs lane_b JSON end-to-end
```
...
- Lane B drives the same config through the tool registry (in-process for
  `test_parity.py`; through the `evt-cli` subprocess in
...
- Root `pyproject.toml`: `packages/*` workspace glob already picks the
  package up; add `hangar-evt` to root dependencies + workspace source,
  and `packages/evt/tests` to `testpaths`.
- `docker/docker-compose.yml`: `evt` service, host port **8004** (next
  free after oas=8000, ocp=8001, pyc=8002, omd=8003), in-container 8000,
  `EVT_TRANSPORT/HOST/PORT` env, `./hangar_data/evt:/data` volume.
- CI: nothing new beyond the pins file — `setup-upstream.sh --required`
  picks up the new entry.

## Phase 5 — docs, skills, guardrails

- `packages/evt/CLAUDE.md`: constraints doc, ~30 lines, covering:
  ...
  - Ports: evt=8004. Testing command.
- `packages/evt/skills/evt-cli-guide/`: SKILL.md + commands.md +
  ...
  convention.
- Root `.claude/CLAUDE.md`: add `hangar.evt` to the namespace list,
  `packages/evt/` to the source layout, run/test commands.

## Test inventory

- `packages/evt/tests/`: `test_tools_vehicle.py`, `test_tools_mission.py`,
  `test_tools_analysis.py`, `test_tools_session.py`, `test_validators.py`,
  `test_viz.py`, `test_evt_study_runner.py`, `test_golden.py` — fast,
  using the `test_all` template; fixtures copied from the pyc conftest
  pattern (artifact/provenance isolation, session reset).
- `packages/evt/examples/mission_segments/tests/`: parity + CLI suite
...
|------|-------------|------|
| 0 | upstream pin + packaging patch + smoke import | `uv sync` green, `import evt` works |
...
| 2 | full tool surface, validation, viz, server resources/prompts | unit tests green; server starts; `evt-cli list-tools` complete |
| 3 | three-lane example suite | parity + golden + CLI-script tests green |
| 4 | docker + root wiring | `docker compose up evt` serves on 8004; root `uv run pytest` includes evt |
