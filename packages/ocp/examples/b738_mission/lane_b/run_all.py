"""Lane B: Run the B738 JSON script(s) in-process and print a summary.

Can also be run via CLI:
    uv run ocp-cli --pretty run-script lane_b/reserve_mission.json
"""
...
def main():
    ...
    for name, r in results.items():
        ...
        if "fuel_burn_kg" in r:
            print(f"  Block fuel: {r['fuel_burn_kg']:.1f} kg")
        if "total_fuel_with_reserve_kg" in r:
            print(f"  Total fuel (incl. reserve): {r['total_fuel_with_reserve_kg']:.1f} kg")
        if "OEW_kg" in r and r["OEW_kg"] is not None:
