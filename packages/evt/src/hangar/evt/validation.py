"""Physics and numerics validation for evt results.
...
from hangar.evt.config.limits import (
    BATT_MASS_FRAC_MAX,
    BATT_MASS_FRAC_MIN,
    DISK_LOADING_MAX,
    DISK_LOADING_MIN,
...
def _check_battery_mass_fraction(results: dict) -> ValidationFinding:
    ...
    # Mission results carry MTOW at the top level as max_takeoff_mass_kg; sizing
    # results carry it (also top level) as sized_mtow_kg. Check both so the
    # fraction check actually fires on a run_sizing result, not just mission.
    mtow = results.get("max_takeoff_mass_kg") or results.get("sized_mtow_kg")
