...
Mirrors the pyc/ocp session tools, adapted for evt. Most of the
...
from hangar.evt.state import artifacts as _artifacts, sessions as _sessions
...
async def visualize(
    ...
    """Generate a visualization plot for an evt analysis run.
    ...
    if plot_type not in EVT_PLOT_TYPES:
        raise ValueError(
            f"Unknown plot_type {plot_type!r}. Supported: {sorted(EVT_PLOT_TYPES)}"
        )
    ...
    plot_result = await asyncio.to_thread(
        generate_evt_plot, plot_type, run_id, results, case_name, save_dir,
    )
