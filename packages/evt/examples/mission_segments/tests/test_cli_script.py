"""CLI evaluation: run the Lane B JSON workflows through evt-cli script mode.
...
def _run_script(script_name: str, out_path: Path) -> list:
    """Run a lane_b JSON workflow via evt-cli and return parsed step results."""
    script = _LANE_B / script_name
    proc = subprocess.run(
        [
            sys.executable, "-c", "from hangar.evt.cli import main; main()",
    ...
    assert proc.returncode == 0, f"evt-cli failed:\n{proc.stderr}"
