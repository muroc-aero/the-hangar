...
Evaluates hangar-evt against the direct evtolpy API using the three core
...
- **lane_b/** -- the hangar-evt MCP tool layer. `run_all.py` builds the full
  ...
  workflows as `evt-cli run-script` scripts.
...
- `tests/test_cli_script.py` -- runs the lane_b JSON workflows through the real
  `evt-cli` in a subprocess (the CLI evaluation).
