...
Mirrors the pyc/ocp session tools, adapted for Aviary.
...
from hangar.avy.state import artifacts as _artifacts, sessions as _sessions
...
async def set_requirements(
    requirements: Annotated[
        ...
        "path uses dot notation into results (e.g. "
        "'performance.total_fuel_mass_lbm'). "
        "operator: '<', '<=', '>', '>=', '==', '!='. "
        "Example: [{\"path\": \"performance.gross_mass_lbm\", \"operator\": \"<\", "
        "\"value\": 175000}]",
    ],
...
async def record_conclusion(
    ...
    Returns ``{conclusion_id, run_id, verdict, narrative, metrics, requirements}``
...
async def reset(
    ...
    """Clear all aircraft and cached state for the session.
...
async def get_detailed_results(
    ...
    In 'standard' mode, returns the full results including the mission
    timeseries, design summary, and performance data.
    """
    run_id = await _resolve_run_id(run_id, session_id)
    user = get_current_user()
    artifact = await asyncio.to_thread(_artifacts.get, run_id, session_id, user)
    ...
    if detail_level == "summary":
        flat = dict(results.get("performance", {}))
        flat.update(results.get("optimizer", {}))
        return {
            "run_id": run_id,
            "detail_level": "summary",
            "results": {k: v for k, v in flat.items() if not isinstance(v, (list, dict))},
        }
    else:
        return {
            "run_id": run_id,
...
async def visualize(
    ...
    """Generate a visualization plot for an Aviary sizing run.
    ...
    plot_result = await asyncio.to_thread(
        generate_avy_plot, plot_type, run_id, results, case_name, save_dir,
    )
