"""Lane A: raw upstream OAS-in-Aviary coupled sizing -- the oracle.

Upstream's own integration, driven exactly as its
``run_OAS_wing_mass_example.py`` does: ``OASWingMassBuilder`` +
the Level-3 sequence + the post-``setup()`` ``set_val`` block (values
verbatim). Only the mission phase_info is imported from
...
def run() -> dict:
    """Coupled OAS wing-mass sizing; return headline metrics."""
    ...
    from hangar.avy.config.missions_oas_wing import phase_info as _phase_info

    phase_info = copy.deepcopy(_phase_info)

    old_cwd = os.getcwd()
    workdir = tempfile.mkdtemp(prefix="avy_lane_a_oas_")
    os.chdir(workdir)
    try:
        _clear_problem_names()
        prob = av.AviaryProblem()
        prob.load_inputs(DECK, phase_info)
        prob.load_external_subsystems([OASWingMassBuilder()])
