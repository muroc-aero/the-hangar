"""Lane A: Caravan mission with direct-coupled VLM using raw OpenConcept/OAS.

Replaces PolarDrag with a direct-coupled VLM drag group that runs the
full OAS VLM solver at every Newton iteration. This is the reference
implementation for the direct-coupled slot example -- no omd dependency.
"""
...
from hangar.omd.slots import _DirectVLMDragGroup


class CaravanDirectVLMModel(om.Group):
    """Caravan aircraft model with direct-coupled VLM drag."""
    ...
    def setup(self):
        ...
        # Direct-coupled VLM drag replaces PolarDrag
        self.add_subsystem(
            "drag",
            _DirectVLMDragGroup(
                nn=nn,
...
class CaravanDirectVLMAnalysis(om.Group):
    """Analysis group with direct-VLM-aware aircraft data dict."""
    ...
    def setup(self):
        ...
        self.add_subsystem(
            "analysis",
            BasicMission(num_nodes=nn, aircraft_model=CaravanDirectVLMModel),
            promotes_inputs=["*"],
            promotes_outputs=["*"],
        )
...
def run() -> dict:
    """Run Caravan mission with direct-coupled VLM drag and return key results."""
    nn = MISSION["num_nodes"]

    prob = om.Problem(reports=False)
    prob.model = CaravanDirectVLMAnalysis()
