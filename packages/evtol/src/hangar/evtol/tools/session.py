...
Mirrors the pyc/ocp session tools, adapted for evtol. Most of the
implementation is reused from hangar.sdk.
"""
...
from hangar.evtol.state import artifacts as _artifacts, sessions as _sessions
...
async def set_requirements(
    requirements: Annotated[
        ...
        "path uses dot notation into results (e.g. 'totals.battery_mass_kg' or "
        "'sized_mtow_kg'). operator: '<', '<=', '>', '>=', '==', '!='. "
        "Example: [{\"path\": \"totals.total_mission_energy_kw_hr\", "
        "\"operator\": \"<\", \"value\": 200}]",
    ],
...
async def record_conclusion(
    ...
    The per-requirement verdicts are auto-derived by evaluating the session's
    persisted requirements (set via set_requirements / configure_session)
    against this run's results, so they cannot drift from the numbers.
    """
...
async def reset(
    ...
    """Clear the working vehicle config and cached state for the session.
...
async def list_artifacts(
    ...
    analysis_type: Annotated[str | None, "Filter: 'mission', 'sizing', 'sweep'"] = None,
...
async def get_detailed_results(
    ...
    In 'standard' mode, returns the full results.
    """
    run_id = await _resolve_run_id(run_id, session_id)
    user = get_current_user()
    artifact = await asyncio.to_thread(_artifacts.get, run_id, session_id, user)
...
async def visualize(
    ...
    plot_type: Annotated[
        str,
        "Plot type -- one of: segment_energy, segment_power, mass_breakdown, "
        "mtow_convergence, sweep",
    ],
    session_id: Annotated[str | None, "Session hint for faster artifact lookup"] = None,
    case_name: Annotated[str, "Human-readable label for the plot title"] = "",
    output: Annotated[
        str | None,
        "Override visualization output mode: 'inline' (PNG), 'file' (save only), "
        "'url' (dashboard URL). When None, uses session default.",
    ] = None,
) -> list:
    """Generate a visualization plot for an evtol analysis run.

    Available plot types:
      segment_energy    -- per-segment mission energy (kW*hr) bar chart
      segment_power     -- per-segment average electric power (kW) bar chart
      mass_breakdown    -- component empty-mass breakdown bar chart
      mtow_convergence  -- MTOW guess vs iteration (requires a run_sizing artifact)
      sweep             -- metric vs swept parameter (requires a sweep artifact)
    ...
    if plot_type not in EVTOL_PLOT_TYPES:
        raise ValueError(
            f"Unknown plot_type {plot_type!r}. Supported: {sorted(EVTOL_PLOT_TYPES)}"
        )
    ...
    plot_result = await asyncio.to_thread(
        generate_evtol_plot, plot_type, run_id, results, case_name, save_dir,
    )
