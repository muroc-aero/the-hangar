"""evtol CLI registry -- builds the tool registry for the hangar CLI framework."""
...
def build_evtol_registry() -> dict[str, Callable]:
    """Import evtol tool functions and build name -> function map."""
    from hangar.evtol import server  # noqa: F401
    from hangar.evtol.tools import session as _session_tools
...
def main() -> None:
    ...
    set_registry_builder(build_evtol_registry)
    ...
    _cli_main(
        prog="evtol-cli",
        description="evtol CLI -- run eVTOL sizing and mission analysis from the command line.",
