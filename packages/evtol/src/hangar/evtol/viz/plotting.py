"""evtol-specific plot generators.

Each plot function accepts ``(run_id, results, case_name, *, save_dir)`` and
returns a ``PlotResult``. ``generate_evtol_plot`` routes by ``plot_type``,
...
from hangar.evtol.results import SEGMENT_KEYS, SEGMENT_LABELS, MASS_COMPONENTS

EVTOL_PLOT_TYPES = frozenset({
    "segment_energy",
    "segment_power",
    "mass_breakdown",
    "mtow_convergence",
...
def generate_evtol_plot(
    ...
    """Generate an evtol plot by type. Returns a PlotResult."""
    if plot_type not in EVTOL_PLOT_TYPES:
        raise ValueError(
            f"Unknown evtol plot_type {plot_type!r}. Supported: {sorted(EVTOL_PLOT_TYPES)}"
        )
