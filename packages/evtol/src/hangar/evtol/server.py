"""evtol MCP server -- FastMCP entry point.
...
from hangar.evtol.tools.vehicle import (
    define_vehicle,
...
from hangar.evtol.tools.mission import configure_mission
...
from hangar.evtol.tools.session import (
    configure_session,
    delete_artifact,
...
# Re-export for tests
from hangar.evtol.state import sessions as _sessions, artifacts as _artifacts  # noqa: F401

# ---------------------------------------------------------------------------
# Register evtol plot types with the SDK viewer infrastructure
# ---------------------------------------------------------------------------
...
register_plot_generator(EVTOL_PLOT_TYPES, generate_evtol_plot)
...
mcp = FastMCP(
    ...
    host=os.environ.get("EVTOL_HOST", "127.0.0.1"),
...
workflows, and the resources (evtol://reference, evtol://workflows) for lookup.""",
)
...
from hangar.evtol.tools.resources import (  # noqa: E402
    artifact_by_run_id,
    reference_guide,
    workflow_guide,
)

mcp.resource("evtol://reference", description="Parameter reference for all evtol MCP tools")(reference_guide)
mcp.resource("evtol://workflows", description="Step-by-step workflows for common analysis tasks")(workflow_guide)
mcp.resource("evtol://artifacts/{run_id}", description="Retrieve a saved analysis artifact by run_id")(artifact_by_run_id)

# ---------------------------------------------------------------------------
# Register MCP prompts
# ---------------------------------------------------------------------------

from hangar.evtol.tools.prompts import (  # noqa: E402
...
def main():
    """Console-script entry point for evtol-server."""
    ...
    run_server_main(
        ...
        tool="evtol",
        env_prefix="EVTOL",
        # 8000=oas, 8001=ocp, 8002=pyc, 8003=omd, 8004=evtol
        # (matches the docker-compose host ports)
        default_port=8004,
        description="evtol MCP Server",
    )
